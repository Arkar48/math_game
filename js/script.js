...
let gameresult = ['Correct!','Game over!','Next game','Start again',"time's up"];
let gameExit = ['Are you sure wanted to exit ?', 'yes','no'];
const aboutcontent = ['Evaluate the quiz and choose a correct answer.',
...
function createQuestion(op,randomNumbers){
  ...
  divforAns.classList.add("answer","mt-4","d-flex","justify-content-between");
  const show = `${randomNumbers[0]} ${op[0]} ${randomNumbers[1]} ${op[1]} ${randomNumbers[2]}`;
  const equation = `${randomNumbers[0]} ${op[0].replace("×", "*").replace("÷", "/")} ${randomNumbers[1]} ${op[1].replace("×", "*").replace("÷", "/")} ${randomNumbers[2]}`;
